...
class CollectionService {
  ...
  Future<SyncResult> _executeSyncStrategy(SyncOperation operation) async {
    try {
      ...
      final SyncStrategy strategy = isImmediateSync 
      ...
      } else if (operation is CollectionUpdateOperation && operation.localId != null) {
        await markCollectionDirty(operation.localId!);
      }
  ...
  Future<Collection> createCollection({
    ...
    await _executeSyncStrategy(CollectionCreateOperation(
      name: name.trim(),
      description: description?.trim(),
      visibility: visibility,
  ...
  Future<Collection> updateCollection({
    ...
    if (name != null && name.trim() != existing.name) {
      await db.update(
        'links',
        {
          ...
          'is_dirty': 1, // Mark links as dirty since collection reference changed
    ...
    // Execute sync strategy
    await _executeSyncStrategy(CollectionUpdateOperation(
      localId: id,
  ...
  Future<void> deleteCollection(int id, {String deleteMode = 'move_links'}) async {
    final db = await AppDb.instance();
    ...
    await _executeSyncStrategy(CollectionDeleteOperation(
      localId: id,
  ...
  Future<void> assignLinkToCollection(int linkId, String? collectionName) async {
    final db = await AppDb.instance();
  ...
  String? _validateCollectionData(String name, String? description, String visibility) {
  ...
  Future<List<Tag>> _getCollectionTags(int collectionId) async {
    ...
    return results.map((row) => Tag.fromName(row['tag_name'] as String)).toList();
  }
  ...
  Future<Map<String, dynamic>> _syncLinkAssignments() async {
    ...
    try {
      ...
      final localCollections = await db.query('collections', where: 'remote_id IS NOT NULL');
      ...
      for (final collectionRow in localCollections) {
        ...
        try {
          ...
          (results['errors'] as List<String>).add('Error syncing links for collection ${collection.name}: $e');
        }
      }
      ...
      try {
        ...
        (results['errors'] as List<String>).add('Error syncing uncategorized links: $e');
      }

    } catch (e) {
      (results['errors'] as List<String>).add('Error syncing link assignments: $e');
    }
  ...
  Future<Map<String, dynamic>> syncUp() async {
    ...
    try {
      ...
      if (collectionsResult['errors'] != null) {
        (results['errors'] as List<String>).addAll(collectionsResult['errors'] as List<String>);
      }
      ...
      if (linksResult['errors'] != null) {
        (results['errors'] as List<String>).addAll(linksResult['errors'] as List<String>);
      }
  ...
  Future<Map<String, dynamic>> syncDown({bool forceOverwrite = false}) async {
    ...
    if (!forceOverwrite) {
      ...
      if (hasUnsynced) {
        throw Exception('Unsynced local changes detected. Sync up first or use forceOverwrite.');
      }
    }
    ...
    try {
      ...
      final collectionsResult = await _syncCollectionsDown(forceOverwrite: forceOverwrite);
      ...
      if (linkAssignmentResult['errors'] != null) {
        (results['errors'] as List<String>).addAll(linkAssignmentResult['errors'] as List<String>);
      }
  ...
  Future<Map<String, dynamic>> fullSync({bool resolveConflicts = false}) async {
    ...
    try {
      ...
      // Overall success
      results['success'] = (syncUpResult['success'] ?? false) && (syncDownResult['success'] ?? false);
      
      if (syncUpResult['errors'] != null) {
        (results['errors'] as List<String>).addAll(syncUpResult['errors'] as List<String>);
      }
      if (syncDownResult['errors'] != null) {
        (results['errors'] as List<String>).addAll(syncDownResult['errors'] as List<String>);
      }
        ...
        if (collection.remoteId == null) {
          ...
          try {
            ...
            if (response['success'] == true && response['collection'] != null) {
              ...
              (results['errors'] as List<String>).add('Failed to create collection "${collection.name}": ${response['message'] ?? 'Unknown error'}');
            }
          } catch (e) {
            String errorMessage = 'Failed to create collection "${collection.name}": $e';
            ...
            if (e.toString().contains('409')) {
              ...
              try {
                ...
                if (listResponse['success'] == true && listResponse['collections'] != null) {
                  ...
                  if (existingCollection != null) {
                    ...
                    errorMessage = 'Collection "${collection.name}" returned 409 but does not exist on server';
                    (results['errors'] as List<String>).add(errorMessage);
                  }
                } else {
                  errorMessage = 'Collection "${collection.name}" returned 409 but could not retrieve collection list to verify';
                  (results['errors'] as List<String>).add(errorMessage);
                }
              } catch (linkError) {
                errorMessage = 'Collection "${collection.name}" returned 409 and failed to verify/link: $linkError';
                (results['errors'] as List<String>).add(errorMessage);
              }
            } else if (e.toString().contains('400')) {
              errorMessage = 'Invalid collection data for "${collection.name}" (400 Bad Request)';
              (results['errors'] as List<String>).add(errorMessage);
            } else {
              (results['errors'] as List<String>).add(errorMessage);
            }
          ...
          if (response['success'] == true) {
            ...
            (results['errors'] as List<String>).add('Failed to update collection "${collection.name}": ${response['message'] ?? 'Unknown error'}');
          }
        }
      } catch (e) {
    ...
    for (final row in newLinks) {
      try {
        ...
        if (response['success'] == true && response['link'] != null) {
          ...
          if (remoteId != null) {
            ...
            if (link.collection != null && link.collection!.isNotEmpty) {
              ...
              if (collectionResult.isNotEmpty) {
                final collectionRemoteId = collectionResult.first['remote_id'] as String?;
          ...
          (results['errors'] as List<String>).add('Failed to create link "${link.title ?? link.url}": ${response['message'] ?? 'Unknown error'}');
        }
      } catch (e) {
        // Handle 409 conflicts for links (duplicate URL) - just mark as warning, not error
        if (e.toString().contains('409')) {
          (results['errors'] as List<String>).add('Warning: Link may already exist (409 conflict) - skipping');
        } else {
      ...
      try {
        ...
        if (link.remoteId != null) {
          ...
          if (link.collection != null && link.collection!.isNotEmpty) {
            ...
            if (collectionResult.isNotEmpty) {
              collectionRemoteId = collectionResult.first['remote_id'] as String?;
            }
          }
          ...
          if (response['success'] == true) {
            ...
            (results['errors'] as List<String>).add('Failed to assign link to collection: ${response['message'] ?? 'Unknown error'}');
          }
        }
      } catch (e) {
        ...
        if (e.toString().contains('409')) {
          try {
            ...
            (results['errors'] as List<String>).add('Link collection assignment returned 409 and failed to mark as synced: $updateError');
          }
        } else {
  ...
  Future<Map<String, dynamic>> _syncCollectionsDown({bool forceOverwrite = false}) async {
    final db = await AppDb.instance();
    final results = {'updated': 0, 'errors': <String>[]};
    ...
    try {
      ...
      if (response['success'] == true && response['collections'] != null) {
        ...
        final apiCollectionIds = apiCollections
            .map((c) => c['id']?.toString())
        ...
        if (forceOverwrite) {
          ...
          for (final localCollection in localCollections) {
            ...
            if (remoteId != null && !apiCollectionIds.contains(remoteId)) {
              ...
              await db.delete('collection_tags', where: 'collection_id = ?', whereArgs: [collectionId]);
        ...
        for (final apiCollection in apiCollections) {
          try {
            ...
            if (remoteId == null || remoteId.isEmpty) {
              (results['errors'] as List<String>).add('Collection has no ID, skipping: ${apiCollection['name'] ?? 'Unknown'}');
              continue;
            }

            final collectionName = apiCollection['name'] as String?;
            if (collectionName == null || collectionName.isEmpty) {
              (results['errors'] as List<String>).add('Collection has no name, skipping ID: $remoteId');
              continue;
            }
            ...
            if (forceOverwrite) {
              ...
              if (existingResult.isEmpty) {
                ...
                final existingCollection = Collection.fromMap(existingResult.first);
                final updatedCollection = Collection.fromApiResponse(apiCollection);
                ...
                // Update tags
                await _updateCollectionTags(existingCollection.id!, updatedCollection.tags);
                
                results['updated'] = (results['updated'] as int) + 1;
              }
            }
          } catch (e) {
            (results['errors'] as List<String>).add('Error processing collection from API: $e');
          }
        }
      } else {
        (results['errors'] as List<String>).add('Failed to fetch collections from API: ${response['message'] ?? 'Unknown error'}');
      }
    } catch (e) {
      (results['errors'] as List<String>).add('Error fetching collections from API: $e');
    }
  ...
  Future<void> markCollectionSynced(int collectionId, {String? remoteId}) async {
    final db = await AppDb.instance();
    final updateData = {
      'is_dirty': 0,
      'last_synced_at': DateTime.now().toIso8601String(),
  ...
  Future<Map<String, dynamic>> getSyncStatus() async {
    ...
    final totalCollectionsResult = await db.rawQuery('SELECT COUNT(*) as count FROM collections');
    final totalLinksResult = await db.rawQuery('SELECT COUNT(*) as count FROM links');
    
    // Count dirty items
    final dirtyCollectionsResult = await db.rawQuery('SELECT COUNT(*) as count FROM collections WHERE is_dirty = 1');
    final dirtyLinksResult = await db.rawQuery('SELECT COUNT(*) as count FROM links WHERE is_dirty = 1');
    
    // Count never synced items
    final unsyncedCollectionsResult = await db.rawQuery('SELECT COUNT(*) as count FROM collections WHERE last_synced_at IS NULL');
    final unsyncedLinksResult = await db.rawQuery('SELECT COUNT(*) as count FROM links WHERE last_synced_at IS NULL');
    
    // Get last sync times
    final lastCollectionSyncResult = await db.rawQuery('SELECT MAX(last_synced_at) as last_sync FROM collections WHERE last_synced_at IS NOT NULL');
    final lastLinkSyncResult = await db.rawQuery('SELECT MAX(last_synced_at) as last_sync FROM links WHERE last_synced_at IS NOT NULL');
  ...
  Future<Map<String, dynamic>> getCollectionStatistics() async {
    ...
    // Total collections
    final totalResult = await db.rawQuery('SELECT COUNT(*) as count FROM collections');
    final total = totalResult.first['count'] as int;
    
    // Collections by visibility
    final publicResult = await db.rawQuery("SELECT COUNT(*) as count FROM collections WHERE visibility = 'public'");
    final privateResult = await db.rawQuery("SELECT COUNT(*) as count FROM collections WHERE visibility = 'private'");
    final hiddenResult = await db.rawQuery("SELECT COUNT(*) as count FROM collections WHERE visibility = 'hidden'");
    
    // Collections with links
    final withLinksResult = await db.rawQuery('SELECT COUNT(*) as count FROM collections WHERE link_count > 0');
    
    // Average links per collection
    final avgLinksResult = await db.rawQuery('SELECT AVG(link_count) as avg FROM collections WHERE link_count > 0');
    
    // Most popular collection
    final popularResult = await db.rawQuery('SELECT name, link_count FROM collections ORDER BY link_count DESC LIMIT 1');
    
    // Total links in collections vs uncategorized
    final totalLinksResult = await db.rawQuery('SELECT COUNT(*) as count FROM links');
    final categorizedLinksResult = await db.rawQuery('SELECT COUNT(*) as count FROM links WHERE collection IS NOT NULL AND collection != ""');
    ...
    return {
      ...
      'average_links_per_collection': avgLinksResult.first['avg'] as double? ?? 0.0,
      'most_popular_collection': popularResult.isNotEmpty ? {
      ...
      'links': {
        ...
        'categorization_percentage': totalLinks > 0 ? (categorizedLinks / totalLinks * 100).round() : 0,
      },
    };
  }
  ...
  Future<List<String>> validateDatabaseIntegrity() async {
    ...
    for (final row in invalidCountsResult) {
      issues.add('Collection "${row['name']}" has incorrect link count: stored=${row['link_count']}, actual=${row['actual_count']}');
    }
    ...
    for (final row in invalidLinksResult) {
      issues.add('Link "${row['url']}" references non-existent collection "${row['collection']}"');
    }
